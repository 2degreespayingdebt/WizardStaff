...
io.on('connection', (socket) => {
    ...
    // Make pick
    socket.on('draft:pick', async (data) => {
        try {
            const { draft, pick } = await draftModel.makePick(data.draftId, data.teamId, data.playerId);
            ...
            socket.emit('draft:error', { error: error.message });
        }
    });
    // Pause/resume draft (commissioner only)
    socket.on('draft:pause', async (data) => {
