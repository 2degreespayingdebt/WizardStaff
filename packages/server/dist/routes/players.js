...
router.get('/search', async (req, res) => {
    try {
        ...
        const players = await playerModel.searchPlayers(q, limit ? parseInt(limit) : undefined);
        res.json(players);
    }
    catch (error) {
    ...
    try {
        ...
        for (const player of players) {
            try {
                ...
                await playerModel.createDrinker(player.name, player.description || null, player.profileImage || null, player.team || null, player.projectedPoints || null, player.adp || null);
                results.created++;
            }
            catch (err) {
                results.failed++;
                results.errors.push(`Failed to create ${player.name}: ${err.message}`);
            }
        }
        res.json(results);
    }
