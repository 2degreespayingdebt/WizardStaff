...
router.post('/', async (req, res) => {
    try {
        ...
        const league = await leagueModel.createLeague(name, commissionerId, {});
        // Create team for commissioner
        const teamName = commissionerId + ' Team';
        await teamModel.createTeam(league.id, commissionerId, teamName);
...
router.post('/join', async (req, res) => {
    try {
        ...
        const team = await teamModel.createTeam(league.id, userId, userId + ' Team');
...
router.post('/seasons/:seasonId/teams/:teamId/drinks', async (req, res) => {
    try {
        const { change } = req.body;
        if (typeof change !== 'number') {
            return res.status(400).json({ error: 'Change must be a number' });
        }
        const result = await seasonModel.updateDrinkCount(req.params.seasonId, req.params.teamId, change);
        res.json(result);
    }
    catch (error) {
...
// Activate season
router.post('/seasons/:seasonId/activate', async (req, res) => {
    try {
...
router.delete('/seasons/:seasonId/teams/:teamId', async (req, res) => {
    try {
...
router.put('/seasons/:seasonId/teams/:teamId/order', async (req, res) => {
    try {
...
router.post('/:id/teams', async (req, res) => {
    try {
        ...
        // Use authenticated user or fall back to demo user
        const userId = req.userId || 'fcb5a616-deec-4153-baa9-3f8659f805a1';
        const team = await teamModel.createTeam(league.id, userId, teamName);
...
// Update team order
router.put('/:id/teams/order', async (req, res) => {
    try {
        const { order } = req.body;
