  ...
  try {
    ...
    if (existingLeague.rows.length === 0) {
      ...
      const leagueResult = await query(
        `INSERT INTO leagues (name, commissioner_id, max_teams) VALUES ($1, $2, $3) RETURNING id`,
        ['🍺 Drinking Buddies', userId, 10]
      );
    ...
    // Create Alpha teams
    const alphaTeamIds: string[] = [];
    for (let i = 0; i < alphaTeams.length; i++) {
      const result = await query(
        `INSERT INTO teams (league_id, manager_id, name) VALUES ($1, $2, $3) RETURNING id`,
        [leagueId, managers[i], alphaTeams[i]]
      );
    ...
    for (let i = 0; i < betaTeams.length; i++) {
      const result = await query(
        `INSERT INTO teams (league_id, manager_id, name) VALUES ($1, $2, $3) RETURNING id`,
        [leagueId, managers[i % managers.length], betaTeams[i]]
      );
    ...
    const alphaResult = await query(
      `INSERT INTO seasons (league_id, name, is_active) VALUES ($1, $2, $3) RETURNING id`,
      [leagueId, 'Alpha', true]
    );
    ...
    for (let i = 0; i < alphaTeamIds.length; i++) {
      ...
      await query(
        `INSERT INTO season_teams (season_id, team_id, seed, drink_count) VALUES ($1, $2, $3, $4)`,
        [alphaSeasonId, alphaTeamIds[i], i + 1, drinkCount]
      );
    }
    ...
    for (let i = 0; i < betaTeamIds.length; i++) {
      ...
      await query(
        `INSERT INTO season_teams (season_id, team_id, seed, drink_count) VALUES ($1, $2, $3, $4)`,
        [betaSeasonId, betaTeamIds[i], i + 1, drinkCount]
      );
    }
